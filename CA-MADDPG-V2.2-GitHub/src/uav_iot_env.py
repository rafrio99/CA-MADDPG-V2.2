...
class UAVIOTParallelEnv(ParallelEnv):
    ...
    def __init__(self, config_path: str = "configs/maddpg_base.yaml"):
        ...
        self.calibration = env_cfg["calibration"]

        self.task_size_bits_per_unit = float(
            self.calibration["task_size_bits_per_unit"]
        )
        self.use_distance_rate_penalty = bool(
            self.calibration["use_distance_rate_penalty"]
        )
        self.downlink_output_fraction = float(
            self.calibration["downlink_output_fraction"]
        )
        self.maximum_compute_power_watts = float(
            self.calibration["maximum_compute_power_watts"]
        )
        self.battery_voltage = float(
            self.calibration["battery_voltage"]
        )
    ...
    def step(self, actions: Dict[str, np.ndarray]):
        ...
        distance_factor = (
            1.0 / (1.0 + local_distance_m / 5_000.0)
            if self.use_distance_rate_penalty
            else 1.0
        )
        ...
        input_bits = (
            float(task["task_size_MB"])
            * self.task_size_bits_per_unit
        )
        ...
        downlink_bits = (
            input_bits * self.downlink_output_fraction
        )
        ...
        compute_power_watts = max(
            1.0,
            self.maximum_compute_power_watts
            * cpu_fraction**3,
        )
